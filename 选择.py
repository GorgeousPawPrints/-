    ...
    for i in range(1, 25):
        ...
        for j in range(2, 206):
            try:
                ...
                now_data_timu = \
                    now_result.xpath(
                        "//*[@id[substring(., string-length(.) - string-length('content') + 1) = 'content']]")[
                        j - 2]
                ...
                if now_data_anser in select_anser or now_data_timu in select_timu:
                    continue
                else:
                    select_anser.append(now_data_anser)
                    select_timu.append(now_data_timu)
                print(select_timu.__len__())
            except:
                break
    ...
    for i in range(1,1000):
        try:
            with open(f'选择.txt', mode='a+', encoding='gbk') as f:
                f.write(f"第{i}题")
                f.write(select_timu[i])
                f.write(select_anser[i])
            f.close()
        except:
            break
    with open(f'选择.txt', 'r') as f:
        content = f.read()
    f.close()
    # 删除所有空格和换行符
    content = content.replace('&nbsp;', '').replace(' ', '').replace('\n', '').replace('</div>', '').replace('<div>',
