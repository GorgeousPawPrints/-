...
def analys(page):
    for i in range(1,25):
        ...
        result_tiankong = html.xpath('/html/body/div/div/div[3]/div[4]/div')[0]
        # result_wenda = html.xpath('/html/body/div/div/div[3]/div[4]/div')[0]
        # /html/body/div/div/div[3]/div[4]/div/div[2]/div[2]
        for j in range(2,100):
            try:
                ...
                # /html/body/div/div/div[3]/div[4]/div/div[2]/div[2]/div[2]/div[5]/text()
                # now_data_anser = now_result.xpath('./div[2]/div[2]/div[5]')[0]
                # print(HTML.unescape(etree.tostring(now_data_anser).decode('gbk')))
                ...
                now_data_timu = now_data_timu.replace('<pstyle="margin-left:28px;text-indent:0">', '').replace(
                    '<spanstyle=";font-family:宋体;font-size:14px">', '')
...
if __name__ == '__main__':
    ...
    for i in range(1, 1000):
        try:
            with open(f'简答.txt', mode='a+', encoding='gbk') as f:
                f.write(f"第{i}题")
                f.write(tiankong[i])
            f.close()
        except:
            break
    with open(f'简答.txt', 'r') as f:
        content = f.read()
    f.close()
